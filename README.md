...
| 平台 | 文件 | 说明 |
|------|------|------|
| macOS (Apple Silicon) | `Extral-macos.dmg` | FFmpeg 内嵌于 .app 包内 |
| Windows | `Extral-windows-amd64.exe` | FFmpeg 内嵌于 exe，首次运行自动释放 |
...
直接双击运行 `Extral-windows-amd64.exe`，首次启动会自动释放 FFmpeg 到缓存目录（`%LocalAppData%/Extral/ffmpeg/`），之后启动即用。
...
FFmpeg 二进制通过 Git LFS 管理，已在 `thirdparty/` 中。克隆仓库后执行：

```bash
git lfs pull
```

如需手动更新，从以下地址下载对应平台的静态编译版本：
...
# macOS .app + .dmg（FFmpeg 打入 .app 包）
task package:darwin:dmg

# Windows .exe（FFmpeg 通过 go:embed 嵌入单个 exe）
...
- **自动更新**：GitHub Releases API + ghfast.top 镜像加速
